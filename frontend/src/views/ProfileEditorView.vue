...
import { describeSongSelection, newProfile, profileInputFromProfile, sanitizeProfile } from '@/domain/profile'
  ...
  <form v-if="!loading" class="editor-layout" @submit.prevent="save(false)">
    ...
    <div class="editor-sections">
      <section id="general" class="card editor-section"><header><span>1</span><div><h2>Allgemein & Verbindungen</h2><p>WorshipTools ist die Quelle, ChurchTools das Ziel.</p></div></header><div class="form-grid">
        <label class="span-2"><span>Profilname</span><input v-model="form.name" required maxlength="120" /><small>Dieser Name erscheint in der Profilübersicht, in Läufen und Benachrichtigungen.</small></label>
        <label><span>WorshipTools-Quelle</span><select v-model="form.source_connection_id" required><option value="" disabled>Auswählen …</option><option v-for="connection in sourceConnections" :key="connection.id" :value="connection.id">{{ connection.name }}</option></select><small>Liefert die Services und die Reihenfolge der Songs.</small><small v-if="!sourceConnections.length">Lege zuerst eine WorshipTools-Verbindung an.</small></label>
        <label><span>ChurchTools-Ziel</span><select v-model="form.target_connection_id" required @change="loadMetadata"><option value="" disabled>Auswählen …</option><option v-for="connection in targetConnections" :key="connection.id" :value="connection.id">{{ connection.name }}</option></select><small>Stellt Kalender, Songdatenbank und Ziel-Agenda bereit.</small><small v-if="!targetConnections.length">Lege zuerst eine ChurchTools-Verbindung an.</small></label>
        <label class="span-2"><span>Synchronisationsverhalten</span><select v-model="form.sync_mode"><option value="source_changes_only">Nur bei Änderungen in WorshipTools (Standard)</option><option value="enforce_source">WorshipTools bei jedem Lauf erzwingen</option></select><small>Im Standardmodus wird nach dem ersten erfolgreichen Sync erst wieder geschrieben, wenn sich Songs, Reihenfolge oder relevante Profileinstellungen ändern. „Erzwingen“ gleicht ChurchTools bei jedem Lauf erneut an.</small></label>
        <label class="check-card span-2"><input v-model="form.enabled" type="checkbox" /><span><strong>Profil aktiv</strong><small>Aktiviert die automatischen Läufe nach dem unten festgelegten Zeitplan.</small></span></label>
      </div></section>
      ...
      <section id="matching" class="card editor-section"><header><span>2</span><div><h2>Event-Matching</h2><p>Mehrdeutige Treffer werden sicher übersprungen und als partiell gemeldet.</p></div></header><div class="form-grid">
        <label><span>Zuordnung</span><select v-model="form.match_mode"><option value="exact_time">Exakte Startzeit</option><option value="date_only">Nur lokales Datum</option></select><small>Exakte Startzeit vergleicht denselben Zeitpunkt; lokales Datum ignoriert die Uhrzeit.</small></label>
        <label><span>Vorausschau</span><div class="input-suffix"><input v-model.number="form.lookahead_days" type="number" min="1" max="365" required /><span>Tage</span></div><small>So viele Tage in die Zukunft werden bei jedem Lauf berücksichtigt.</small></label>
        <label><span>WorshipTools-Zeitzone</span><input v-model="form.source_timezone" required placeholder="Europe/Berlin" /><small>Bestimmt das lokale Datum der WorshipTools-Services beim Datumsabgleich.</small></label>
        <label><span>ChurchTools-Zeitzone</span><input v-model="form.target_timezone" required placeholder="Europe/Berlin" /><small>Bestimmt das lokale Datum der ChurchTools-Events beim Datumsabgleich.</small></label>
      </div>
      <p class="form-hint">Innerhalb einer Regel müssen alle gesetzten Filter passen; mehrere Regeln werden als Alternativen ausgewertet.</p>
      <div class="repeat-list"><article v-for="(rule, index) in form.event_rules" :key="index" class="repeat-card"><header><h3>Regel {{ index + 1 }}</h3><button v-if="form.event_rules.length > 1" class="link-button danger-text" type="button" @click="removeRule(index)">Entfernen</button></header><div class="form-grid">
        <label><span>Name enthält</span><input v-model="rule.name_contains" placeholder="z. B. Gottesdienst" /><small>Optionaler Text, der im Namen des ChurchTools-Events vorkommen muss.</small></label><label><span>Sichere Regex (optional)</span><input v-model="rule.name_regex" maxlength="256" placeholder="^Sonntag" /><small>Kann allein stehen; mit „Name enthält“ müssen beide Namensfilter passen.</small></label>
        <label><span>Kalender-IDs</span><select v-if="calendars.length" v-model="rule.calendar_ids" multiple><option v-for="option in calendars" :key="option.id" :value="option.id">{{ option.name }}</option></select><input v-else :value="rule.calendar_ids.join(', ')" placeholder="Kommagetrennte IDs" @input="rule.calendar_ids = ($event.target as HTMLInputElement).value.split(',').map(v => v.trim()).filter(Boolean)" /><small>Beschränkt die Regel auf diese ChurchTools-Kalender; leer bedeutet alle Kalender.</small></label>
        <label><span>Campus-IDs</span><select v-if="campuses.length" v-model="rule.campus_ids" multiple><option v-for="option in campuses" :key="option.id" :value="option.id">{{ option.name }}</option></select><input v-else :value="rule.campus_ids.join(', ')" placeholder="Kommagetrennte IDs" @input="rule.campus_ids = ($event.target as HTMLInputElement).value.split(',').map(v => v.trim()).filter(Boolean)" /><small>Beschränkt die Regel auf diese ChurchTools-Campusse; leer bedeutet alle Campusse.</small></label>
      ...
      <section id="placement" class="card editor-section"><header><span>3</span><div><h2>Platzierung in ChurchTools</h2><p>Zugeordnete Song-Items werden verwaltet; fremde Header und Texte bleiben erhalten.</p></div></header>
        ...
        <div class="repeat-list"><article v-for="(placement, index) in form.placements" :key="index" class="repeat-card"><header><h3>Zielbereich {{ index + 1 }}</h3><button v-if="form.placements.length > 1" class="link-button danger-text" type="button" @click="removePlacement(index)">Entfernen</button></header><div class="form-grid">
          ...
          <label><span>Ende exklusiv (optional)</span><input v-model.number="placement.song_end" type="number" step="1" placeholder="alle übrigen" :aria-describedby="`song-selection-${index}`" /><small>−1 lässt den letzten Song aus; leer nimmt alle übrigen.</small></label>
          <p :id="`song-selection-${index}`" class="form-hint song-selection-description span-2" aria-live="polite"><strong>Auswahl:</strong> {{ describeSongSelection(placement.song_start, placement.song_end) }}</p>
        ...
        <div class="form-grid standalone"><label class="check-card span-2"><input v-model="form.create_missing_songs" type="checkbox" /><span><strong>Fehlende Songs automatisch erstellen</strong><small>Fehlt ein eindeutiger Treffer, legt der Sync Song und Standard-Arrangement in ChurchTools an.</small></span></label>
          <label v-if="form.create_missing_songs"><span>ChurchTools-Songkategorie</span><select v-if="songCategories.length" v-model.number="form.song_category_id" required><option :value="null" disabled>Auswählen …</option><option v-for="option in songCategories" :key="option.id" :value="Number(option.id)">{{ option.name }}</option></select><input v-else v-model.number="form.song_category_id" type="number" min="1" required placeholder="Kategorie-ID" /><small>Diese Kategorie erhalten automatisch neu angelegte ChurchTools-Songs.</small></label>
          <label v-if="form.create_missing_songs"><span>Name des Standard-Arrangements</span><input v-model="form.arrangement_name" required maxlength="50" /><small>Wird verwendet, wenn für einen Song ein neues Arrangement angelegt werden muss.</small></label>
        </div>
        <div class="form-grid standalone">
          ...
          <label><span>Agenda-Titel</span><input v-model="form.agenda_item_defaults.title" maxlength="100" placeholder="Optionaler eigener Titel" /><small>Überschreibt optional den Titel jedes verwalteten Song-Items, maximal 100 Zeichen.</small></label>
          <label><span>Verantwortlich</span><input v-model="form.agenda_item_defaults.responsible" maxlength="1000" placeholder="z. B. [Worship Leader]" /><small>Setzt Text oder ChurchTools-Platzhalter für jedes verwaltete Song-Item, maximal 1.000 Zeichen.</small></label>
          <label><span>Dauer</span><div class="input-suffix"><input v-model.number="form.agenda_item_defaults.duration" type="number" min="0" max="86400" step="1" placeholder="0" /><span>Sekunden</span></div><small>Setzt die Dauer jedes verwalteten Song-Items zwischen 0 Sekunden und 24 Stunden.</small></label>
          <label class="span-2"><span>Agenda-Notiz</span><textarea v-model="form.agenda_item_defaults.note" rows="3" maxlength="4000" placeholder="Optionale Hinweise für das Team"></textarea><small>Hinterlegt dieselbe optionale Notiz an jedem verwalteten Song-Item, maximal 4.000 Zeichen.</small></label>
        </div>
      </section>

      <section id="schedule" class="card editor-section"><header><span>4</span><div><h2>Zeitplan</h2><p>Das Mindestintervall beträgt 30 Minuten.</p></div></header><fieldset class="segmented"><legend>Art des Zeitplans</legend><button type="button" :class="{ active: form.schedule_type === 'interval' }" @click="form.schedule_type = 'interval'">Intervall</button><button type="button" :class="{ active: form.schedule_type === 'cron' }" @click="form.schedule_type = 'cron'">Cron-Ausdruck</button></fieldset><p class="form-hint schedule-hint">Ein Intervall startet regelmäßig; ein Cron-Ausdruck legt bestimmte Uhrzeiten und Wochentage fest.</p><div class="form-grid schedule-fields">
        <label v-if="form.schedule_type === 'interval'"><span>Alle</span><div class="input-suffix"><input v-model.number="form.interval_minutes" type="number" min="30" max="10080" required /><span>Minuten</span></div><small>Bestimmt den Abstand zwischen automatischen Läufen; manuelle Starts bleiben möglich.</small></label>
        <label v-else><span>Cron-Ausdruck</span><input v-model="form.cron_expression" required placeholder="0 * * * *" /><small>Wird in der Zielzeitzone ausgewertet; „0 6 * * *“ startet täglich um 06:00 Uhr.</small></label>
      </div></section>
      ...
      <section id="notify" class="card editor-section"><header><span>5</span><div><h2>Benachrichtigungen</h2><p>In-App ist die kanonische Quelle. Telegram ist veraltet und standardmäßig aus.</p></div></header><div class="check-grid">
        <label class="check-card"><input :checked="true" type="checkbox" disabled /><span><strong>In-App</strong><small>Speichert wichtige Ergebnisse immer im Notification Center dieses Workspaces.</small></span></label><label class="check-card"><input v-model="form.notification_preferences.email" type="checkbox" /><span><strong>E-Mail</strong><small>Sendet aktivierte Meldungen an die E-Mail-Adresse deines Kontos.</small></span></label><label class="check-card"><input v-model="form.notification_preferences.web_push" type="checkbox" /><span><strong>Web Push</strong><small>Zeigt aktivierte Meldungen auf Browsergeräten mit erteilter Freigabe.</small></span></label><label class="check-card deprecated"><input v-model="form.notification_preferences.telegram" type="checkbox" /><span><strong>Telegram</strong><small>Veralteter optionaler Kanal, der eine konfigurierte Integration benötigt.</small></span></label>
        <label class="check-card"><input v-model="form.notification_preferences.notify_new_songs" type="checkbox" /><span><strong>Neue Songs</strong><small>Meldet, wenn WorshipTools-Songs neu in ChurchTools angelegt wurden.</small></span></label><label class="check-card"><input v-model="form.notification_preferences.notify_success" type="checkbox" /><span><strong>Erfolgreiche Läufe</strong><small>Meldet auch fehlerfreie Läufe und kann bei kurzen Intervallen häufig auslösen.</small></span></label>
      </div></section>
