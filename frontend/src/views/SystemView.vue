...
const manualRunCooldownSeconds = ref<AdminWorkspace['manual_run_cooldown_seconds']>(1800)
...
function edit(workspace: AdminWorkspace): void {
  ...
  manualRunCooldownSeconds.value = workspace.manual_run_cooldown_seconds
}

function cooldownLabel(seconds: AdminWorkspace['manual_run_cooldown_seconds']): string {
  if (seconds === 0) return 'Kein Cooldown'
  if (seconds === 1800) return 'Standard · 30 Minuten'
...
async function saveQuotas(): Promise<void> {
  ...
  try {
    const updated = await api.patch<AdminWorkspace>(`/admin/workspaces/${editing.value.id}/quotas`, {
      ...
      manual_run_cooldown_seconds: manualRunCooldownSeconds.value,
    })
    const index = workspaces.value.findIndex((item) => item.id === updated.id)
    if (index >= 0) workspaces.value[index] = updated
    editing.value = null
    toasts.show('success', 'Workspace-Nutzung aktualisiert', 'Die Änderung wurde im Audit-Log protokolliert.')
  } catch (cause) {
    toasts.show('error', 'Workspace-Nutzung konnte nicht geändert werden', errorMessage(cause))
  } finally {
    saving.value = false
  }
}
...
<template>
  <PageHeader title="Systemverwaltung" eyebrow="Plattform" description="Technischer Zustand und Nutzungseinstellungen aller Workspaces auf dieser Instanz."><button class="button button-secondary" type="button" @click="load">Aktualisieren</button></PageHeader>
  ...
  <div v-if="editing" class="dialog-backdrop" @click.self="editing = null"><section ref="quotaDialog" class="dialog quota-dialog" role="dialog" aria-modal="true" aria-labelledby="quota-title" tabindex="-1"><header><div><p class="eyebrow">Plattform-Administration</p><h2 id="quota-title">Nutzung für {{ editing.name }}</h2></div><button class="icon-button" type="button" aria-label="Dialog schließen" @click="editing = null">×</button></header><form class="stack-form" @submit.prevent="saveQuotas"><label><span>Maximale Sync-Profile</span><input v-model.number="profileQuota" type="number" min="1" max="1000" required /><small>Aktuell verwendet: {{ editing.profile_count }}</small></label><label><span>Maximale Mitglieder</span><input v-model.number="memberQuota" type="number" min="1" max="10000" required /><small>Aktuell verwendet: {{ editing.member_count }}</small></label><label><span>Cooldown für manuelle Runs</span><select v-model.number="manualRunCooldownSeconds"><option :value="1800">Standard – 30 Minuten</option><option :value="900">15 Minuten</option><option :value="300">5 Minuten</option><option :value="0">Kein Cooldown</option></select><small>Gilt je Sync-Profil; parallele Runs bleiben ausgeschlossen.</small></label><div class="alert alert-warning"><strong>Plattformweite Änderung</strong><span>Diese Aktion wird mit deinem Konto im Audit-Log gespeichert.</span></div><div class="dialog-actions"><button class="button button-secondary" type="button" @click="editing = null">Abbrechen</button><button class="button button-primary" type="submit" :disabled="saving">{{ saving ? 'Speichert …' : 'Nutzung speichern' }}</button></div></form></section></div>
</template>
